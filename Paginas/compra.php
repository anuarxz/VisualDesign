...
<head>
    ...
    <title>VisualDesign - Compra</title>
        ...
        <div class="container">
          ...
          <div class="collapse navbar-collapse" id="navbarTogglerDemo02">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
              ...
              <li class="nav-item">
                <a class="nav-link"  aria-current="page" href="../Paginas/catalogo.php">Catálogo</a>
              </li>
              <li class="nav-item">
...
session_start();
  ...
  // Y comprobamos que el usuario se haya autentificado
   if (!isset($_SESSION['correo'])) {
    ...
    <img class="d-block mx-auto mb-4" src="../Imagenes/logo2.png" alt="" width="72" height="57">
    <h1 class="display-5 fw-bold">No se puede comprar</h1>
    <div class="col-lg-6 mx-auto">
      <p class="lead mb-4">Registrate o inicia sesión para realizar la compra.</p>
      <div class="d-grid gap-2 d-sm-flex justify-content-sm-center">
        <a href="../Paginas/IniciarSesion.php"><button type="button" class="btn btn-primary btn-lg px-4 gap-3">Iniciar sesión</button></a>
        <a href="../Paginas/registro.php"><button type="button" class="btn btn-outline-secondary btn-lg px-4">Registrarse</button></a>
      </div>
    </div>
  </div><br><br><br><br><br>';
  }else{
    try {
      ...
      if (!isset($error)) {
        ...
        if (isset($_POST['compra'])) {
          ...
          $sql = "SELECT Nombre FROM Productos WHERE ID=".$_REQUEST["idcompra"];
          $resultado = $dwes->query($sql);
              $row = $resultado->fetch();
              while ($row != null) {
                  $nombre = $row['Nombre'];
                  $fecha = date('d/m/Y H:i:s');
                  $sql2 = "INSERT INTO Compras(Fecha, NombreProducto, CorreoCliente) VALUES ('$fecha', '$nombre','$correo')";
          ...
          <div class='col-lg-6 mx-auto'>
            <p class='lead mb-4'>Compra realizada correctamente</p>
            <div class='d-grid gap-2 d-sm-flex justify-content-sm-center'>
              <a href='../Paginas/pedidos.php'><button type='button' class='btn btn-primary btn-lg px-4 gap-3'>Ver pedidos</button></a>
            </div>
          </div>
        </div>";
        ...
        if ($resultado) {
            ...
            while ($row != null) {
                ...
                <h2 class='pb-2 border-bottom'>Comprar producto</h2>
              ...
              <form name='input' action='compra.php' method='post' ENCTYPE='multipart/form-data'>
              <input type='hidden' name='idcompra' value='$id'>
              <h4 class='pb-2 border-bottom'><b>Nombre del producto:</b> $nombre</h4>
              <h4 class='pb-2 border-bottom'><b>Descripción:</b> $descripcion</h4>
              <h4 class='pb-2 border-bottom'><b>Categoria:</b> $categoria</h4>
              <h4 class='pb-2 border-bottom'><b>Precio:</b> $precio</h4>
              </div>
              <div class='d-grid gap-2 d-md-flex justify-content-center mb-4 mb-lg-3'>
                      <button type='submit' class='btn btn-primary' name='compra'>Realizar compra</button>
                      </form>
                    </div><br>" ;
